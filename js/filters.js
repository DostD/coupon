...
function refilter() {
    ...
    for (var i = 0; i<productCards.length; i++) {
        ...
        ok === true ? productCards[i].style.display = 'inline-block' : productCards[i].style.display = 'none';
    }
}
...
function validateDate (card , state){
    ...
    cardDate < state ? check = true : check = false
    return check;
}
...
function validatePrice(card, state){
  var check = false;
  if(card.dataset.price >= state.price.from && card.dataset.price <= state.price.to){
        check = true;
    }
   return check;
}
